---
layout: blog.njk
title: 'Getting Started with Luanti'
tags: ['blog', 'rss', 'topic_index']
date: 2025-12-07
---

## About

[Luanti][2] is an open source voxel game engine that lends itself particularly well to buidling things together with friends. Naturally, I run a server.

Here you will find my notes on running [MineGame][3] on Luanti, for friends and family.
Except where otherwise noted, my server is typically [Debian][debian] and my usual client is my [SteamDeck][steamdeck].

These are my notes on my setup. As always, this is ostensibly for your benefit. 

More honestly, these notes are so I can re-build my setup later after I inevitably mess it up while tinkering.


[debian]: https://www.debian.org/
[steamdeck]: https://store.steampowered.com/steamdeck
[2]: https://www.luanti.org/en/
[3]: https://content.minetest.net/packages/Minetest/minetest_game/

## Getting Started

- [Install Luanti on SteamDeck](https://blog.rubenwardy.com/2022/12/02/minetest-steam-deck/)
- [Choose some Mods](/blog/luanti/mods)
- [Play with Robots!](/blog/luanti/robots)
- [Play with Trains!](/blog/luanti/trains)
- [Playing together offline](/blog/luanti/offline)

## Running a Server

- [Run a Dedicated Server](/blog/luanti/server)
- [Backup your world files](/blog/luanti/backup)
- [My Luanti Command Cheat Sheet](/blog/luanti/cheatsheet)
- [My Luanti Invoke Files](/blog/luanti/invoke)
